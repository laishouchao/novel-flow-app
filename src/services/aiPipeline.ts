...
请直接输出摘要，不要输出解释。`,
  ...
  private parseProjectFromMarkdown(markdown: string): Partial<NovelProject> {
    ...
    // 提取类型
    const genreMatch = markdown.match(/类型[：:]\s*(.+)/);
    if (genreMatch) {
    ...
    // 提取核心种子（一句话故事）
    const loglineMatch = markdown.match(/一句话故事.*?\n\n?(.+)/s);
    if (loglineMatch) {
      result.coreSeed = loglineMatch[1].trim().slice(0, 500);
    }

    // 提取世界观
    const worldMatch = markdown.match(/###\s*世界观\s*\n\n?(.+?)(?=\n###|\n##|$)/s);
    if (worldMatch) {
      result.worldBuilding = worldMatch[1].trim();
    }

    // 提取核心冲突
    const conflictMatch = markdown.match(/###\s*核心冲突\s*\n\n?(.+?)(?=\n###|\n##|$)/s);
    if (conflictMatch) {
      result.conflict = conflictMatch[1].trim();
    }

    // 提取角色动力学
    const charMatch = markdown.match(/##\s*角色设定\s*\n\n?([\s\S]+?)(?=\n##\s*叙事|$)/s);
    if (charMatch) {
      result.characterDynamics = charMatch[1].trim();
    }

    // 提取情节架构
    const plotMatch = markdown.match(/##\s*叙事结构\s*\n\n?(.+?)(?=\n##|$)/s);
    if (plotMatch) {
      result.plotArchitecture = plotMatch[1].trim();
    }
  ...
  private parseConsistencyResult(response: string): ConsistencyResult {
    ...
    // 检查是否通过
    if (response.includes('未发现一致性问题') || response.includes('Canon 检查通过')) {
      result.passed = true;
    } else {
      result.passed = false;
    }
